...
#'
#' @export
svmtHMM <- function(y, y0=0.2, m, gmax, theta_init=NULL, nIS=1e3){
...
lforward.svmtHMM=function(x, m, gbmax, p, y0){
  ...
  foo <- delta*1/beg*dt((x[1]-p[1]-p[2]*yy[1]-p[3]*beg^2)/beg, p[7])
...
svmt.HMM.quantiles=function(x, res.time, m, gbmax, p, lalpha, y0){
...
#' @export
svmtHMM.pseudo_residuals=function(object, ytest){
  ...
  lalpha=lforward.svmtHMM(x=c(ytrain, ytest), m=m, gbmax=gbmax, p=p, y0=y0)
  ...
  for(k in 1:length(ytest)){
    ...
    psressvn[k]=svmt.HMM.quantiles(x=c(ytrain, ytest), res.time=length(ytrain)+k,
