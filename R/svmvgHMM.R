svmvg.pn2pw = function(beta, mu, phi, sigma, nu){
  lbeta1 = beta[1]
...
fillallprobs_vg=function(x,beg,beta,nu,y){
...
svmvg.mllk=function(parvect, y, y0, m, gmax){
  ...
  xx<-y
  yy<-c(y0,y[1:(ny-1)])
  allprobs = outer(xx,sey,"fillallprobs_vg", beta=p[1:3], nu=p[7], yy)
...
#' @export
svmvgHMM=function(y, y0=0.2, m, gmax, theta_init=NULL, nIS=1e3){
  ...
  eigenvalues = rep(0, 7)
  cont = 0
  parvect0 = svmvg.pn2pw(beta0, mu0, phi0, sigma0, nu0)
...
#' @export
summary.svmvgHMM <- function(object, ...) {
  ISdiag(
...
#' @export
info_crit.svmvgHMM <- function(object, ...) {
  ...
  for (j in seq_along(object$Weigth)) {
    Dbar <- Dbar +
      object$Weigth[j] *
      svmvg.mllk(object$X[j, ], object$y, object$y0, object$m, object$gmax)
  }
...
#' @export
logvol.svmvgHMM <- function(object, plot=FALSE, ...) {

  X      <- object$X
  Weigth <- object$Weigth

  # Transform parameters from working space to natural space
  Thetas <- t(apply(X = X, MARGIN = 1, FUN = svmvg.pw2pn))
...
lforward.svmvgHMM=function(x, m, gbmax, p, y0){
  ...
  foo <- delta*1/beg*pdf_vg((x[1]-p[1]-p[2]*yy[1]-p[3]*beg^2)/beg, p[7])
...
svmvg.HMM.quantiles=function(x, res.time, m, gbmax, p, lalpha, y0){
...
#' @export
svmvgHMM.pseudo_residuals=function(object, ytest){
  ...
  lalpha=lforward.svmvgHMM(x=c(ytrain, ytest), m=m, gbmax=gbmax, p=p, y0=y0)
  ...
  for(k in 1:length(ytest)){
    ...
    psressvn[k]=svmvg.HMM.quantiles(x=c(ytrain, ytest), res.time=length(ytrain)+k,
